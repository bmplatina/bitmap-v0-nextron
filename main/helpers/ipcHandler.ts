 ...
 */
class ipcHandle {
    constructor(bIsProduction: boolean, MAIN_WINDOW: BrowserWindow, processPlatform: string) {
        // Electron
        ...
        // neDB state store
        this.gameInstallInfoDbPath = this.bIsProd ? join(app.getPath('userData'), 'gameInstallInfo.db') : './gameInstallInfo.db';
        this.gameInstallInfoDb = new Datastore({filename: this.gameInstallInfoDbPath, autoload: true});

        this.settingsDbPath = this.bIsProd ? join(app.getPath('userData'), 'settings.db') : './settings.db';
        this.settingsDb = new Datastore({filename: this.settingsDbPath, autoload: true});
    }
    ...
    initializeIpc() {
        ...
        ipcMain.handle('check-executable-or-app', (event, dirPath: string): boolean => {
            try {
                const extensionName = this.platformName === 'darwin'
                    ? '.app/'
                    : '.exe';
        ...
        ipcMain.handle('game-install-info-get-by-index', (_, gameIdIndex: number): Promise<any> => {
            return new Promise((resolve, reject) => {
                this.gameInstallInfoDb.findOne({ gameId: gameIdIndex }, (err, docs: GameInstallInfo) => {
                    if (err) {
                        console.error(err);
                        reject(err);
                    } else {
        ...
        ipcMain.handle('game-install-info-delete', (_, gameIdIndex: number): Promise<any> => {
            return new Promise((resolve, reject) => {
                this.gameInstallInfoDb.remove({ gameId: gameIdIndex }, (err, numRemoved) => {
                    if (err) {
                        reject(err);
                    } else {
                        this.gameInstallInfoDb.loadDatabase();
        ...
        ipcMain.handle('game-install-info-update', (event, gameIdIndex: number, gameInstallInfo: GameInstallInfo) => {
            // 조건에 맞는 데이터 업데이트
            return new Promise((resolve, reject) => {
                this.gameInstallInfoDb.update({ gameId: gameIdIndex }, { $set: gameInstallInfo }, { upsert: false }, (err, numReplaced) => {
                    if (err) {
                        reject(err);
                    } else {
                        this.gameInstallInfoDb.loadDatabase();
        ...
        ipcMain.handle('settings-update', (event, newSettings: Settings) => {
            return new Promise((resolve, reject) => {
                this.settingsDb.update({ id: 0 }, { $set: newSettings }, { upsert: true }, (err, numReplaced) => {
                    if (err) {
                        reject(err);
                    } else {
